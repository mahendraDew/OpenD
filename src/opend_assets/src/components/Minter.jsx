...
function Minter() {
  ...
  const [loaderHidden, setLoaderHiddend] = useState(true);

  async function onSubmit(data) {
    setLoaderHiddend(false);
    const name = data.name;
    const image = data.image[0];
    const imageByteData = [...new Uint8Array(await image.arrayBuffer())];   //arrayBuffer() can be use to generate genic binary data

    const newNFTID = await opend.mint(imageByteData, name);
    console.log(newNFTID.toText());
    setNFTPrincipal(newNFTID);

    setLoaderHiddend(true);
  }
